 ...
 * @param {type} destParentNode
 */
function walkHtml(parentNode, destParentNode) {
    var i;
    var childNodes;
    var childNode;
    var childNodeWoChildren;
...
//        console.log("\n");
        ...
        childNodeWoChildren = childNode.cloneNode(false);

        switch (childNode.nodeType) {
            case Node.ELEMENT_NODE:
                processElementNodeHtml(childNode, destParentNode);
                walkHtml(childNode, childNodeWoChildren)
                break;

            case Node.TEXT_NODE:
                // Copy the node into the destination DOM
                destParentNode.appendChild(childNodeWoChildren);
                walkHtml(childNode, childNodeWoChildren)
                break;
            ...
            default:
                console.err('[Error: Unknown node type = ' + childNode.nodeType + ']');
 ...
 * @param {Element} destParentNode the node in the destination DOM to append to
 * @returns {string} The optional modified report text.
 */
function processElementNodeHtml(elm, destParentNode) {
    ...
    switch (elm.nodeName) {
        ...
        case "P":           // Paragraph
            ...
            destParentNode.appendChild(elm);
            break;

            /* ==== 8.1.3 Report Template Fields ==== */

        case "TEXTAREA":
            appendFormElement(elm, destParentNode)
            break;
        ...
        default:
            // Ignore element
            console.err('[Element not specified in MRRT -> ignored: ' + elm.nodeName + ']');
    }
 ...
 * @returns {string} The optional modified report text.
 */
function appendFormElement(elm, destParentNode) {
    var textNode;
    var textValue;
    var labelText;
    ...
    textValue = getElmValue(elm);
    textNode = elm.ownerDocument.createTextNode(textValue);
    destParentNode.appendChild(textNode);
}
